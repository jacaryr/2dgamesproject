...
ImageFactory::~ImageFactory() {
  ...
  map<std::string, SDL_Surface*>::const_iterator ptr = 
  surfaces.begin();
  while(ptr != surfaces.end()){  
    SDL_FreeSurface(ptr->second);
    ++ptr;
  }
  map<std::string, SDL_Texture*>::const_iterator ptr1 = 
  textures.begin();
  ...
  map<std::string, Image*>::const_iterator ptr2 = 
  images.begin();
  
  while(ptr2 != images.end()){
    std::cout << "deleting " << ptr2->first << std::endl;
    delete ptr2->second;
    ++ptr2;
  }
  ...
  for ( auto textures : multiTextures ) {
    for (unsigned int i = 0; i < textures.second.size(); ++i) {
      SDL_DestroyTexture( textures.second.at(i) );
    }
  }
